...
To begin, install the necessary dependencies: Install devtools and other packages below if needed
```
...
First, load the `Trinotate annotation report (.tsv)` into `R` using the `read_trinotate` function from the `TrinotateR` package. 
This function reads the data into a `data.table` format for easy manipulation.
...
Example Data is freely available at Mendeley Data [http://dx.doi.org/10.17632/3rtbr7c9s8.1](http://dx.doi.org/10.17632/3rtbr7c9s8.1)
...
>Most of the annotations contain mutliple hits in a backtick-delimited list and each hit >contains multiple fields in a caret-delimited list. For example, the second Pfam >annotation below contains two hits and each hit contains a pfam id, symbol, name, >alignment and e-value. The 'split_pfam' function from the 'TrinotateR' package splits >multiple hits and fields, so the second Pfam annotation is now printed in rows 2 and 3 >below.
...
Example Data is freely available at Mendeley Data repository [http://dx.doi.org/10.17632/ycrmntp78w.3](http://dx.doi.org/10.17632/ycrmntp78w.3)
...
### Create pie chart of Pfam domains by total TPM on Ss_SE

```
ggplot(x1_pfam_selected, aes(x = "", y = total_TPM, fill = factor(name, levels = x1_pfam_selected$name))) +
...
Now, create the pie chart for Ss_SE using ggplot2, label the top 20 slices, and display the name labels on the side.
...
quant_data <- read.delim("/home/danysaurio/TrinotateR_Ss_report/quant_Ss_PE.sf", header = TRUE)
```
...
ggplot(y1_pfam_selected, aes(x = "", y = total_TPM, fill = factor(name, levels = y1_pfam_selected$name))) +
  geom_bar(width = 1, stat = "identity", color = "black") +  # Add black border
  coord_polar(theta = "y") +
  theme_minimal() +
  labs(title = "Pie Chart of Pfam Domains by Total TPM",
...
```
Step 2: Create the Pie Chart
Now, create the pie chart for Ss_PE using ggplot2, label the top 20 slices, and display the name labels on the side.
